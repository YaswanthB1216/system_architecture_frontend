...
//       currentList.push(element);
//     } else {
//       if (currentList.length >  result.push(
//     <Box key={`list-${index}`} component={listType} sx={{ paddingLeft: '20px', marginBottom: '16px' }}>
//       {currentList}
//     </Box>
//   ));
...
//   return (
//     <Box display="flex" flexDirection="column" flex={1} height="100%" sx={{ background: 'none' }} overflowY='hidden'>
//       {/* Scrollable Messages Area */}
//       <Box
//         flex={1}
//         sx={{
//           overflowY: 'auto',
//           maxHeight: 'calc(100vh - 120px)', // Adjust based on input bar height and other UI elements
//           p: 2,
...
//               <Box key={message.id} display="flex" justifyContent={message.role === 'user' ? 'flex-end' : 'flex-start'} mb={2}>
//                 <Box
//                   maxWidth={{ xs: '90%', sm: '80%', md: '600px' }} // Responsive maxWidth
//                   sx={getMessageStyle(message.role)}
//                 >
//                   <Box display="flex" alignItems="center" mb={1}>
//                     {message.role === 'user' ? (
...
const formatMessageContent = (content) => {
  ...
  const formattedLines = lines.map((line, index) => {
    ...
    if (isBullet) {
      ...
      return (
        <Box key={index} component="p" sx={{ marginBottom: '8px' }}>
          {line}
        </Box>
      );
    }
    ...
    >
      ...
      <Box
        ...
        sx={{
          ...
          maxHeight: 'calc(100vh - 128px)', // Header (64px) + Input (approx 64px)
          scrollbarWidth: 'thin', // Firefox support
          scrollbarColor: 'transparent transparent', // Firefox thumb and track
          '&::-webkit-scrollbar': {
            width: '8px',
            background:'transparent'
          },
                    ...
                    {message.role === 'user' ? (
                      <User
                        style={{
                          ...
                          width: 16,
                          marginRight: 8,
                          color: getIconColor(message.role),
                        }}
                      />
                    ) : (
                      <Bot
      ...
      <Box
        ...
        gap={2}
        alignItems="center"
        sx={{ background: '#FFFFFF', flexShrink: 0 }}
      >
        <TextField
          fullWidth
          variant="outlined"
      ...
      <Box sx={{display:'flex',justifyContent:'center', padding:'0px 10% 0px 0px'}}>
